{{--
    ...
    ─────────────────────────────────────────
    • Pending banner: matches Transfer-OUT dark-gold design exactly
    • Popup: purple theme, fires ONCE per completed record via localStorage seen-set
    • wire:poll.8000ms="checkNewNotifications" on PHP side
    • Alpine handles all visual state — no double-fire on re-render
--}}

<div
    x-data="transferInNotify()"
    x-init="boot()"
    x-on:transfer-in-new-notification.window="handleNewRecord($event.detail.record)"
    wire:poll.8000ms="checkNewNotifications"
>

{{-- ══════════════════════════════════════════════════════════
     PENDING BANNER — matches Transfer-OUT golden style
══════════════════════════════════════════════════════════ --}}
    ...
    ">
            ...
            <div style="min-width:0;">
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;flex-wrap:wrap;">
                    <span style="font-size:15px;font-weight:800;color:#fde68a;letter-spacing:.01em;">
                        <span x-text="pending"></span>&nbsp;{{ __('message.Transfer-IN') }} {{ __('message.Requests') }}
                    </span>
                    <span x-show="acceptedCount > 0"
                          style="background:rgba(59,130,246,0.20);border:1px solid rgba(59,130,246,0.45);
                        color:#93c5fd;font-size:10px;font-weight:800;padding:2px 10px;
                        border-radius:6px;text-transform:uppercase;letter-spacing:.08em;">
                        {{ __('message.Accepted') }}
                    </span>
                    <span x-show="acceptedCount === 0"
                          style="background:rgba(245,158,11,0.20);border:1px solid rgba(245,158,11,0.45);
        ...
        <div style="display:flex;align-items:center;gap:8px;flex-shrink:0;
            ...
            <span style="font-size:10px;color:rgba(253,230,138,0.45);letter-spacing:.05em;
                font-family:monospace;white-space:nowrap;">LIVE · ↻ 8s</span>
        </div>
    </div>
</div>

{{-- ══════════════════════════════════════════════════════════
     LIVEWIRE → ALPINE sync (pending count)
══════════════════════════════════════════════════════════ --}}
<div
    style="display:none;"
    x-ref="lwSync"
    ...
    wire:key="tin-sync-{{ $pendingCount }}-{{ $acceptedCount }}"
    x-effect="
        pending      = parseInt($refs.lwSync.dataset.pending)  || 0;
        acceptedCount= parseInt($refs.lwSync.dataset.accepted) || 0;
    "
></div>

{{-- ══════════════════════════════════════════════════════════
     POPUP — purple professional, completed only, fires ONCE per record
══════════════════════════════════════════════════════════ --}}
        ...
        >
            ...
            <div style="padding:14px 26px 10px;display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
                ...
                <div style="background:rgba(34,197,94,0.12);border:1px solid rgba(34,197,94,0.35);
                    color:#4ade80;border-radius:999px;padding:3px 12px;
...
@keyframes tinDotBlink  { 0%,100%{opacity:1;box-shadow:0 0 10px rgba(34,197,94,0.9)} 50%{opacity:.3;box-shadow:0 0 4px rgba(34,197,94,0.3)} }
...
function transferInNotify() {
    return {
        ...
        // ── localStorage seen-set key (version-stamped so old keys don't interfere)
        SEEN: 'tin_seen_v3',
        ...
        saveSeenSet(s) {
            let a = [...s];
            if (a.length > 800) a = a.slice(-800); // cap size
        ...
        // ── ENTRY — called by x-on:transfer-in-new-notification.window ───
        handleNewRecord(record) {
            if (!record || !record.id) return;
            // ✅ KEY FIX: guard with localStorage — popup fires ONCE per record id ever
        ...
        speak(rec) {
            ...
            // Voice speech after beep finishes
            setTimeout(() => {
                try {
                    if (!('speechSynthesis' in window)) return;
                    window.speechSynthesis.cancel();
                    const msg = `{{ __('message.Transfer-IN') }} {{ __('message.Completed') }}. `
                        + `Invoice ${rec.invoice_number}. `
                        + `Customer ${rec.customer_name}. `
                        + `Net amount ${rec.currency || ''} ${this.fmt(rec.net_amount)}.`;
                    const u  = new SpeechSynthesisUtterance(msg);
                    u.lang   = document.documentElement.lang || 'en-US';
                    u.rate   = 0.88;
                    u.volume = 1;
                    window.speechSynthesis.speak(u);
                } catch(e) { console.warn('[TransferIN] speak error', e); }
            }, 700);
        },

        // ── boot ──────────────────────────────────────────────────────────
        boot() {
            console.log('[TransferInNotify] Booted — pending:', this.pending);
        },
